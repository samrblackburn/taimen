...
#' @export
tar_target(plotMat, function(LB_obj=NULL, size.axtex=12, size.title=14, size.leg=12, useSmooth=TRUE, Title=NULL) {
  ...
  if (class(LB_obj) == "LB_obj") {
    ...
    mplot <- ggplot(DF, aes(x=Lens, y=Dat)) +
      geom_line(aes(x=Lens, y=SelDat, color=Year), longSel, linewidth=1) +
