...
map_targets <- list(
  ...
  tar_target(locations, read_csv(locations_file) %>% 
               st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326)),
  
  # get bounding box of main map to use on inset map
  tar_target(locations_box, st_bbox(c(xmin = 90, ymin = 41, xmax = 146, ymax = 56), crs = 4326) %>% 
               st_as_sfc() %>% 
               st_cast("LINESTRING")),
  ...
  # create inset map
  tar_target(inset_grob, ggplotGrob(
    ggplot() +
