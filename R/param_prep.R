param_prep_targets <- list(
  ...
  tar_target(
    delger_length_file,
    "data/DelgerLengthAssessmentWorkshop_final.csv",
    format = "file"
  ),
  tar_target(
    onon_length_file,
    "data/OnonLengthAssessmentWorkshop_final.csv",
    format = "file"
  ),
  tar_target(
    eguur_length_file,
    "data/EgUurLengthAssessmentWorkshop_final.csv",
    format = "file"
  ),
  tar_target(
    tugur_length_file,
    "data/TugurLengthAssessmentWorkshop_final.csv",
    format = "file"
  ),
  tar_target(
    karibetsu_length_file,
    "data/KaribetsuLengthAssessmentWorkshop_final.csv",
    format = "file"
  ),
  tar_target(
    koppi_length_file,
    "data/KoppiLengthAssessmentWorkshop_final.csv",
    format = "file"
  ),
  ...
  tar_target(
    rivers,
    list(
      delgermoron = as.matrix(read_csv(
        delger_length_file,
  ...
  tar_target(delger_onon_params, {
    ...
    tibble(
      linf = rlnorm(rows, linf_mean, linf_sd),
  ...
  tar_target(
    ...
    delger_onon_params %>%
      bind_cols(taimen_m) %>%
      mutate(pop = "delgermoron", species = "Hucho taimen")
  ),
  tar_target(
    onon_params,
    delger_onon_params %>%
      bind_cols(taimen_m) %>%
      mutate(pop = "onon", species = "Hucho taimen")
  ),
  tar_target(
    tugur_params,
    tugur_post %>%
      bind_cols(taimen_m) %>%
      mutate(pop = "tugur", species = "Hucho taimen")
  ),
  tar_target(
    eguur_params,
    eguur_post %>%
      bind_cols(taimen_m) %>%
      mutate(pop = "eguur", species = "Hucho taimen")
  ),
  tar_target(
    koppi_params,
    itou_post %>%
      bind_cols(itou_m) %>%
      mutate(pop = "koppi", species = "Parahucho perryi")
  ),
  tar_target(
    karibetsu_params,
    itou_post %>%
      bind_cols(itou_m) %>%
      mutate(pop = "karibetsu", species = "Parahucho perryi")
  ),
  ...
  tar_target(
    ...
    params_mc %>%
      summarise(
        across(
          c(linf, k, m),
          list(mean = ~ signif(mean(.), 3), sd = ~ signif(sd(.), 3))
        ),
        .by = pop
      ) %>%
      mutate(linf_cv = round(linf_sd / linf_mean, 3)) %>%
      rename(river = pop) %>%
      mutate(
  ...
  tar_target(create_tpars, function(species, m, k, linf, linf_cv) {
  ...
  tar_target(create_lvec, function(LB_pars, dat) {
    ...
    LB_pars@BinMax <- ceiling(
      (max(LB_pars@Linf * 1.1, 1.1 * max(dat, na.rm = TRUE))) / 0.5
    ) *
      0.5
    ...
    lvec@LData <- sapply(1:ncol(dat), function(x) {
      hist(dat[, x], breaks = LBins, plot = FALSE)$counts
    })
    lvec@LMids <- hist(dat[, 1], breaks = LBins, plot = FALSE)$mids
    if (length(LB_pars@L_units) > 0) {
      lvec@L_units <- LB_pars@L_units
    }
    return(lvec)
  })
)
