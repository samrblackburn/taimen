...
mc_plot_targets <- list(
  ...
  tar_target(pull_output_dist, function(river, year, log.FM, log.FM.SD, log.SPR, log.SPR.SD, n) {
  ...
  tar_target(mc_plot_df, expand_output(mc_output) %>% 
  ...
  tar_target(mc_spr_plot, ggplot(data = mc_plot_df, aes(x = year)) + 
               facet_wrap( ~ river) +
               ...
               theme(axis.text.x = element_text(angle = -45, hjust = 0),
                     ...
                     legend.position = "top")),
  tar_target(mc_fm_plot, ggplot(data = mc_plot_df, aes(x = year)) + 
