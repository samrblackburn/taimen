...
#' @export
tar_target(plotSize, function(LB_obj=NULL, size.axtex=12, size.title=14, Title=NULL,
                     scales=c("free", "fixed", "free_x", "free_y"), inc.text=FALSE,
                     warn.size=0.8, counts = NULL) {
  ...
  if (!is.null(counts)){
    bplot <- bplot +
      geom_text(data = counts, mapping = aes(x = Inf, y = Inf, label = paste("n=", n)),
                hjust = 1.5, vjust = 1.5, # These adjust the text position slightly inside the panel
                color = "black")
  }
  ...
  if (chk & chk2) { # model has been fitted
    ...
    bplot <- bplot +
      geom_line(aes(x=longDat2$LMids, y=longDat2$PredLen), colour="black", linewidth=1.25)
