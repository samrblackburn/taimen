##modifying plotSize from LBSPR package to be able to customize ggplot aesthetics

plotsize_targets <- list(
  ...
  tar_target(
    ...
    function(
      ...
      if (class(LB_obj) != "LB_obj" & class(LB_obj) != "LB_lengths") {
        stop("Require LB_lengths or LB_obj object", call. = FALSE)
      }
      scales <- match.arg(scales)
      ...
      Ldat <- LB_obj@LData
      if (length(Ldat) < 1) {
        stop("No length data found", call. = FALSE)
      }
      LMids <- LB_obj@LMids
      ...
      longDat$LMids <- LMids
      levelnames <- sort(as.numeric(colnames(Ldat)))
      longDat$Year <- factor(longDat$Year, levels = levelnames)
      ...
      bplot <- ggplot(longDat, aes(x = LMids, y = LBSPR_len)) +
        ...
        geom_bar(stat = "identity", orientation = 'x', fill = "grey70") +
        xlab(XLab) +
        ylab("Number of Fish") +
        theme_bw() +
        theme(
      ...
      if (!is.null(counts)) {
        bplot <- bplot +
          geom_text(
            data = counts,
            mapping = aes(x = Inf, y = Inf, label = paste("n=", n)),
            hjust = 1.5,
            vjust = 1.5, # These adjust the text position slightly inside the panel
            color = "black"
          )
      }
      ...
      chk2 <- FALSE
      if (chk) {
        if (length(LB_obj@pLCatch) > 0) chk2 <- TRUE
      }
      if (chk & chk2) {
        ...
        bplot <- bplot +
          geom_line(
            aes(x = longDat2$LMids, y = longDat2$PredLen),
            colour = "black",
            linewidth = 1.25
          )
        ...
        if (inc.text) {
          if (length(ind) > 0) {
            ...
            if (length(yrs) > 0) {
              text_dat <- data.frame(
                Year = factor(yrs, levels = levels(longDat$Year)),
                LMids = longDat$LMids[0.5 * length(longDat$LMids)],
                LBSPR_len = 0.99 * max(longDat$LBSPR_len),
                lab = "Model didn't converge"
              )
              bplot <- bplot +
                geom_text(data = text_dat, aes(label = lab), size = warn.size)
            }
            # High Selectivity
