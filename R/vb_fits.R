vb_fit_targets <- list(
  ...
  tar_target(
    eguur_age_length_file,
    "data/eguur_age_length.csv",
    format = "file"
  ),
  tar_target(
    tugur_age_length_08_file,
    "data/tugur_age_length_2008.csv",
    format = "file"
  ),
  tar_target(
    tugur_age_length_17_file,
    "data/tugur_age_length_2017.csv",
    format = "file"
  ),
  ...
  tar_target(
    tugur_age_length,
    bind_rows(
      read_csv(tugur_age_length_08_file),
      read_csv(tugur_age_length_17_file)
    )
  ),
  ...
  tar_target(
    itou_priors,
    prior(normal(0, 500), nlpar = "Linf", lb = 0) +
  ...
  tar_target(
    taimen_priors,
    prior(normal(146, 14.9), nlpar = "Linf", lb = 0) +
  ...
  tar_target(
    itou_post,
    as_draws_df(itou_vb_fit) %>%
      select(linf = b_Linf_Intercept, k = b_K_Intercept)
  ),
  tar_target(
    eguur_post,
    as_draws_df(eguur_vb_fit) %>%
      select(linf = b_Linf_Intercept, k = b_K_Intercept)
  ),
  tar_target(
    tugur_post,
    as_draws_df(tugur_vb_fit) %>%
      select(linf = b_Linf_Intercept, k = b_K_Intercept)
  ),
